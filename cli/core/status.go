...
func GetStatus(ctx context.Context, eigenpodAddress string, validatorIndices []uint64, eth *ethclient.Client, beaconClient BeaconClient) EigenpodStatus {
	validators := map[string]Validator{}
	var activeCheckpoint *Checkpoint = nil
	...
	startTime = time.Now()
	checkpointTimestamp, allValidatorsForEigenpod, err := SfGetCheckpointTimestampAndBeaconState(ctx, eigenpodAddress, validatorIndices, eth, beaconClient)
	PanicOnError("failed to fetch checkpoint and beacon state", err)
